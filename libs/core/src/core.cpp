...
bool ForwardSynthesis::forward_synthesis_() {
  ...
  //  bool result = strategy[sdd_formula_id] !=
  //  sdd_manager_false(context_.manager); context_.logger.info("Explored
  //  states: {}",
...
//     context_.print_search_debug("Loop detected for node {}, tagging the
//     node",
//                                 sdd_formula_id);
...
//     sdd_manager_true(context_.manager); context_.indentation -= 1; return
//     success_strategy;
//   }
//
...
//       context_.print_search_debug("No children, {} is failure",
//       sdd_formula_id); path.pop(); context_.discovered[sdd_formula_id] =
//       false; context_.indentation -= 1; return failure_strategy;
//     }
//
...
//       context_.manager); auto env_state_node =
//           SddNodeWrapper(child_it.get_sub(), context_.manager);
//       if (env_state_node.get_type() == STATE) {
//         // OR->OR transition
//         auto formula_next_state =
//         next_state_formula_(env_state_node.get_raw()); auto next_state =
//         formula_to_sdd_(formula_next_state); auto next_state_id =
//         next_state.get_id(); add_transition_(sdd, system_move.get_raw(),
//         next_state); auto next_state_result_it =
...
//                 "system look-ahead: next state {} already discovered,
//                 success", next_state.get_id());
//             path.pop();
//             context_.indentation -= 1;
//             strategy_t strategy;
...
//         context_.print_search_debug("system look-ahead: {} is not a state
//         node",
...
//       context_.print_search_debug("checking system move: {}",
//       system_move_str);
//       ++child_it;
...
//     // do the one-step-lookahead:
//     //  - if already discovered: if success, return, otherwise must be
//     visited
//     //  later
...
//       auto formula_next_state =
//       next_state_formula_(wrapper.begin().get_sub()); context_.indentation -=
//       1; return system_move_(formula_next_state, path);
//     }
...
//   context_.manager); return wrapper;
// }
...
//           // TODO check: not all children of the AND node might have been
//           added
//           // to the subgraph.
//           auto children_by_action =
//           context_.graph.get_successors(predecessor); if (std::all_of(
...
//      print_search_debug("vtree garbage collection with threshold {}
//      completed",
//                         gc_threshold);
